...
#include "Character/AI/Sight/QTAISense_Sight.h"
...
float UQTAISense_Sight::Update()
{
	...
	// sort Sight Queries
	{
		...
		{
			Algo::ForEach(SightQueriesOutOfRange, RecalcScore);
	...
	if (QueryOperations.Num() > 0)
	{
		...
		if (InvalidTargets.Num() > 0)
		{
			...
			for (const auto& TargetId : InvalidTargets)
			{
				...
				OnSightTargetsChanged.Broadcast(ObservedTargets.Find(TargetId)->Target.Get());
